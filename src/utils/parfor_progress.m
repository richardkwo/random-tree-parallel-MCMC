...
if N > 0
    f = fopen([tempdir,'parfor_progress.txt'], 'w');
    if f<0
...
elseif N == 0
    delete([tempdir,'parfor_progress.txt']);
    percent = 100;
    ...
    if ~exist([tempdir,'parfor_progress.txt'], 'file')
        error('parfor_progress.txt not found. Run PARFOR_PROGRESS(N) before PARFOR_PROGRESS to initialize parfor_progress.txt.');
    end
    
    f = fopen([tempdir,'parfor_progress.txt'], 'a');
    fprintf(f, '1\n');
    fclose(f);
    
    f = fopen([tempdir,'parfor_progress.txt'], 'r');
    progress = fscanf(f, '%d');
    fclose(f);
